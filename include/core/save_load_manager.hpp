 ...
 */
    struct SaveLoadResult {
        ...
        SaveLoadResult(const SaveLoadError err, const std::string& msg)
            : success(false), error(err), error_message(msg) {}
    
        static SaveLoadResult Success() { return SaveLoadResult(); }
        static SaveLoadResult Failure(const SaveLoadError err, const std::string& msg) {
 ...
 */
     ...
     */
        static bool IsValidSlotName(const std::string& slot_name);
    ...
    private:
        ...
        nlohmann::json SerializeGameState(ECSWorld& ecs_world, const std::string& tower_name) const;
        bool DeserializeGameState(const nlohmann::json& json, ECSWorld& ecs_world) const;

        static bool ValidateVersion(const nlohmann::json& json);

        static SaveLoadResult CheckDiskSpace(const std::string& path);
        ...
        static constexpr auto SAVE_FILE_EXTENSION = ".tfsave";
        static constexpr auto AUTOSAVE_SLOT_NAME = "_autosave";
        static constexpr auto GAME_VERSION = "0.1.0";
    };
