...
void AchievementsMenu::Update(float delta_time) {
...
void AchievementsMenu::HandleMouse(int mouse_x, int mouse_y, float mouse_wheel) {
...
void AchievementsMenu::SetGameStats(int population, float total_income, int floor_count, float avg_satisfaction) {
...
void AchievementsMenu::RenderHeader() {
    int screen_width = GetScreenWidth();
    int screen_height = GetScreenHeight();
    int menu_x = (screen_width - MENU_WIDTH) / 2;
    int menu_y = (screen_height - MENU_HEIGHT) / 2;
    ...
    int title_width = MeasureText(title, 30);
    DrawText(title, menu_x + (MENU_WIDTH - title_width) / 2, menu_y + 15, 30, GOLD);
    
    // Progress indicator
    if (achievement_manager_) {
        int unlocked = achievement_manager_->GetUnlockedCount();
        int total = achievement_manager_->GetTotalCount();
        float percentage = achievement_manager_->GetProgressPercentage();
        
        std::ostringstream oss;
        oss << unlocked << "/" << total << " (" << std::fixed << std::setprecision(0) << percentage << "%)";
        std::string progress_text = oss.str();
        
        int progress_width = MeasureText(progress_text.c_str(), 16);
...
void AchievementsMenu::RenderAchievementList() {
    ...
    int list_y = menu_y + HEADER_HEIGHT;
    int list_height = MENU_HEIGHT - HEADER_HEIGHT - FOOTER_HEIGHT;
    ...
    auto unlocked = achievement_manager_->GetUnlockedAchievements();
    ...
    auto locked = achievement_manager_->GetLockedAchievements();
...
void AchievementsMenu::RenderAchievementItem(const TowerForge::Core::Achievement& achievement, int y_pos, bool is_unlocked) {
    int screen_width = GetScreenWidth();
    int screen_height = GetScreenHeight();
    int menu_x = (screen_width - MENU_WIDTH) / 2;
    
    Color bg_color = is_unlocked ? Color{40, 40, 50, 255} : Color{30, 30, 35, 255};
    Color text_color = is_unlocked ? WHITE : DARKGRAY;
    Color icon_color = is_unlocked ? GOLD : DARKGRAY;
    ...
    if (is_unlocked) {
        std::string date_str = "Unlocked: " + achievement.GetUnlockDateString();
        DrawText(date_str.c_str(), menu_x + 80, y_pos + 55, 12, GRAY);
    } else {
        std::string progress_str = GetProgressText(achievement);
...
void AchievementsMenu::RenderFooter() {
    ...
    int menu_x = (screen_width - MENU_WIDTH) / 2;
    int menu_y = (screen_height - MENU_HEIGHT) / 2;
    int footer_y = menu_y + MENU_HEIGHT - FOOTER_HEIGHT;
    
    // Footer background
    DrawRectangle(menu_x, footer_y, MENU_WIDTH, FOOTER_HEIGHT, Color{30, 30, 40, 255});
    
    // Instructions
    const char* instructions = "Press ESC to close | Arrow keys or mouse wheel to scroll";
    int instructions_width = MeasureText(instructions, 14);
    DrawText(instructions, menu_x + (MENU_WIDTH - instructions_width) / 2, footer_y + 13, 14, LIGHTGRAY);
}

std::string AchievementsMenu::GetProgressText(const TowerForge::Core::Achievement& achievement) {
    std::ostringstream oss;
