...
void TutorialManager::Update(float delta_time) {
...
void TutorialManager::RenderOverlay() {
    int screen_width = GetScreenWidth();
    int screen_height = GetScreenHeight();
    
    // Draw semi-transparent overlay at top
    int overlay_x = (screen_width - OVERLAY_WIDTH) / 2;
    int overlay_y = 20;
...
void TutorialManager::RenderStepInfo() {
    int screen_width = GetScreenWidth();
    int overlay_x = (screen_width - OVERLAY_WIDTH) / 2;
    int overlay_y = 20;
    
    std::string title = GetStepTitle();
    std::string hint = GetStepHint();
    
    // Draw title
    int title_font_size = 28;
    int title_width = MeasureText(title.c_str(), title_font_size);
    int title_x = overlay_x + (OVERLAY_WIDTH - title_width) / 2;
    DrawText(title.c_str(), title_x, overlay_y + 20, title_font_size, GOLD);
    
    // Draw decorative line
    int line_width = OVERLAY_WIDTH - 80;
    int line_x = overlay_x + 40;
    DrawRectangle(line_x, overlay_y + 55, line_width, 2, GOLD);
    
    // Draw hint
    int hint_font_size = 18;
    int hint_width = MeasureText(hint.c_str(), hint_font_size);
    int hint_x = overlay_x + (OVERLAY_WIDTH - hint_width) / 2;
    DrawText(hint.c_str(), hint_x, overlay_y + 70, hint_font_size, LIGHTGRAY);
    
    // Draw highlighted facility indicator
    std::string required = GetRequiredFacility();
    if (!required.empty()) {
        std::string indicator = "[Highlight: " + required + " button]";
        int indicator_font_size = 16;
        int indicator_width = MeasureText(indicator.c_str(), indicator_font_size);
        int indicator_x = overlay_x + (OVERLAY_WIDTH - indicator_width) / 2;
...
void TutorialManager::RenderProgress() {
    ...
    std::string progress_text = "[Progress: " + std::to_string(current + 1) + "/" + 
                                std::to_string(total_steps) + "]";
    int progress_font_size = 16;
    int progress_width = MeasureText(progress_text.c_str(), progress_font_size);
    int progress_x = overlay_x + (OVERLAY_WIDTH - progress_width) / 2;
    DrawText(progress_text.c_str(), progress_x, overlay_y + 130, progress_font_size, YELLOW);
    
    // Draw progress bar
    int bar_width = OVERLAY_WIDTH - 100;
    int bar_height = 10;
    int bar_x = overlay_x + 50;
    int bar_y = overlay_y + 155;
    
    // Background
    DrawRectangle(bar_x, bar_y, bar_width, bar_height, ColorAlpha(DARKGRAY, 0.5f));
    
    // Progress fill
    float progress = (float)(current + 1) / (float)total_steps;
...
void TutorialManager::RenderButtons() {
    ...
    int skip_x = overlay_x + OVERLAY_WIDTH - BUTTON_WIDTH - 20;
    int skip_y = overlay_y + OVERLAY_HEIGHT + 10;
    
    // Check hover
    int mouse_x = GetMouseX();
    int mouse_y = GetMouseY();
    bool skip_hover = (mouse_x >= skip_x && mouse_x <= skip_x + BUTTON_WIDTH &&
                       mouse_y >= skip_y && mouse_y <= skip_y + BUTTON_HEIGHT);
    
    // Draw button
    Color skip_bg = skip_hover ? ColorAlpha(RED, 0.4f) : ColorAlpha(DARKGRAY, 0.3f);
    Color skip_border = skip_hover ? RED : GRAY;
    ...
    const char* skip_text = "Skip Tutorial";
    int skip_text_size = 18;
    int skip_text_width = MeasureText(skip_text, skip_text_size);
    int skip_text_x = skip_x + (BUTTON_WIDTH - skip_text_width) / 2;
    int skip_text_y = skip_y + (BUTTON_HEIGHT - skip_text_size) / 2;
