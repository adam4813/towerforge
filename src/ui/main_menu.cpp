...
void MainMenu::RenderBackground() {
    ...
    for (int y = 320; y < 580; y += 30) {
        for (int x = 60; x < 100; x += 20) {
            float pulse = sin(animation_time_ * 2.0f + y * 0.1f) * 0.5f + 0.5f;
    ...
    for (int y = 270; y < 580; y += 30) {
        for (int x = screen_width - 100; x < screen_width - 60; x += 20) {
            float pulse = sin(animation_time_ * 2.0f + y * 0.1f + 1.0f) * 0.5f + 0.5f;
            DrawRectangle(x, y, 10, 15, ColorAlpha(YELLOW, 0.2f + pulse * 0.1f));
        }
    }
}
...
void MainMenu::RenderTitle() {
    ...
    // Draw tagline
    const char* tagline = "\"Build, Survive, Thrive!\"";
    int tagline_font_size = 20;
    int tagline_width = MeasureText(tagline, tagline_font_size);
    int tagline_x = (screen_width - tagline_width) / 2;
...
void MainMenu::RenderMenuOptions() {
    ...
    for (size_t i = 0; i < menu_items_.size(); ++i) {
        ...
        Color bg_color = is_selected ? ColorAlpha(GOLD, 0.3f) : ColorAlpha(DARKGRAY, 0.2f);
...
void MainMenu::RenderVersion() {
    ...
    std::string version_text = "Version: " + GetVersion();
    int font_size = 16;
    int text_width = MeasureText(version_text.c_str(), font_size);
...
int MainMenu::HandleMouse(int mouse_x, int mouse_y, bool clicked) {
    int screen_width = GetScreenWidth();
