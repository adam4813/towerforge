...
void GeneralSettingsMenu::Update(float delta_time) {
    animation_time_ += delta_time;
}

void GeneralSettingsMenu::Render() {
    RenderBackground();
    RenderHeader();
    RenderMenuOptions();
}

void GeneralSettingsMenu::RenderBackground() {
    ...
    // Main menu panel
    int menu_height = MENU_START_Y + menu_items_.size() * (MENU_ITEM_HEIGHT + MENU_ITEM_SPACING) + 80;
    int menu_x = (screen_width - MENU_WIDTH) / 2;
    int menu_y = HEADER_Y - 20;
...
void GeneralSettingsMenu::RenderHeader() {
    ...
    const char* title = "GENERAL SETTINGS MENU";
...
void GeneralSettingsMenu::RenderMenuOptions() {
    int screen_width = GetScreenWidth();
    int screen_height = GetScreenHeight();
    ...
    const char* instruction = "UP/DOWN: Navigate | ENTER: Select | ESC: Back";
...
int GeneralSettingsMenu::HandleMouse(int mouse_x, int mouse_y, bool clicked) {
    int screen_width = GetScreenWidth();
