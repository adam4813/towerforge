...
bool UIWindowManager::HandleClick(int mouse_x, int mouse_y) {
...
UIWindow* UIWindowManager::GetWindow(int window_id) {
    for (const auto& window : windows_) {
...
void UIWindowManager::BringToFront(int window_id) {
...
void UIWindowManager::CalculateWindowPosition(UIWindow* window) {
    ...
    if (!windows_.empty()) {
        ...
        if (top_window) {
            Rectangle top_bounds = top_window->GetBounds();
