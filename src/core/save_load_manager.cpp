...
namespace TowerForge::Core {
    ...
    SaveLoadResult SaveLoadManager::SaveGame(const std::string& slot_name,
        ...
        try {
            ...
            const nlohmann::json json = SerializeGameState(ecs_world, tower_name);
        
            // Write to file
            const std::string file_path = GetSavePath(slot_name);
    ...
    SaveLoadResult SaveLoadManager::RenameSlot(const std::string& old_name,
        ...
        try {
            const std::string old_path = GetSavePath(old_name);
            const std::string new_path = GetSavePath(new_name);
    ...
    bool SaveLoadManager::IsValidSlotName(const std::string& slot_name) {
        if (slot_name.empty() || slot_name.length() > 64) {
            return false;
        }
    
        // Check for illegal characters
        for (const char c : slot_name) {
            if (c == '/' || c == '\\' || c == ':' || c == '*' || 
                c == '?' || c == '"' || c == '<' || c == '>' || c == '|') {
                return false;
            }
    ...
    void SaveLoadManager::SetAutosaveEnabled(const bool enabled) {
        autosave_enabled_ = enabled;
        std::cout << "Autosave " << (enabled ? "enabled" : "disabled") << std::endl;
    }

    void SaveLoadManager::SetAutosaveInterval(const float interval) {
        autosave_interval_ = interval;
        std::cout << "Autosave interval set to " << interval << " seconds" << std::endl;
    }

    void SaveLoadManager::UpdateAutosave(const float delta_time, ECSWorld& ecs_world) {
    ...
    nlohmann::json SaveLoadManager::SerializeGameState(ECSWorld& ecs_world,
                                                       const std::string& tower_name) const {
        nlohmann::json json;
        ...
        const flecs::world& world = ecs_world.GetWorld();
            ...
            if (json.contains("research")) {
                ...
                if (research_json.contains("unlocked_nodes")) {
                    for (const auto& node_id : research_json["unlocked_nodes"]) {
                        auto id = node_id.get<std::string>();
    ...
    bool SaveLoadManager::ValidateVersion(const nlohmann::json& json) {
        if (!json.contains("metadata") || !json["metadata"].contains("game_version")) {
            return false;
        }

        const std::string file_version = json["metadata"]["game_version"];
    ...
    SaveLoadResult SaveLoadManager::CheckDiskSpace(const std::string& path) {
        try {
            return SaveLoadResult::Success();
        } catch (const std::exception& e) {
