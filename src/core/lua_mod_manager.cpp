...
namespace TowerForge::Core {
    ...
    void LuaModManager::SetupSandbox() const {
    ...
    void LuaModManager::RegisterAPI() const {
        if (!lua_state_) return;
    ...
    bool LuaModManager::LoadMod(const std::string& filepath) {
        ...
        const int result = luaL_dofile(lua_state_, filepath.c_str());
    ...
    const LuaFacilityType* LuaModManager::GetCustomFacilityType(const std::string& id) const {
        const auto it = custom_facility_types_.find(id);
    ...
    const LuaVisitorType* LuaModManager::GetCustomVisitorType(const std::string& id) const {
        const auto it = custom_visitor_types_.find(id);
    ...
    LuaModManager* LuaModManager::GetManager(lua_State* L) {
        ...
        const auto manager = static_cast<LuaModManager*>(lua_touserdata(L, -1));
        lua_pop(L, 1);
        return manager;
    }
