...
void AudioManager::Shutdown() {
    ...
    for (auto& pair : music_cache_) {
        UnloadMusicStream(pair.second);
    }
    music_cache_.clear();
    
    // Unload all cached sound effects
    for (auto& pair : sfx_cache_) {
...
void AudioManager::PlayMusic(AudioCue cue, bool loop, float fade_in_duration) {
    if (!initialized_) {
        return;
    }
    ...
    Music* music_ptr = nullptr;
    auto it = music_cache_.find(cue);
    
    if (it != music_cache_.end()) {
        music_ptr = &it->second;
    } else {
        // Try to load the music file
        std::string filepath = GetAudioFilePath(cue);
        if (filepath.empty()) {
...
void AudioManager::StopMusic(float fade_out_duration) {
...
void AudioManager::PlaySFX(AudioCue cue, float volume) {
    if (!initialized_) {
        return;
    }
    
    // Try to get from cache or load
    Sound* sound_ptr = nullptr;
    auto it = sfx_cache_.find(cue);
...
void AudioManager::PlayAmbient(AudioCue cue, bool loop, float fade_in_duration) {
    if (!initialized_) {
        return;
    }
...
void AudioManager::StopAmbient(float fade_out_duration) {
...
float AudioManager::GetVolume(AudioType type) const {
    switch (type) {
        case AudioType::Music:
...
bool AudioManager::LoadAudioFile(AudioCue cue, const std::string& filepath, bool is_music) {
...
std::string AudioManager::GetAudioFilePath(AudioCue cue) const {
...
void AudioManager::UpdateMusicFade(float delta_time) {
    if (!music_loaded_) {
        music_fading_ = false;
        return;
    }
    ...
    float target_vol = master_volume_ * music_fade_target_;
...
void AudioManager::UpdateAmbientFade(float delta_time) {
    if (!ambient_loaded_) {
        ambient_fading_ = false;
        return;
    }
    ...
    // Calculate target volume with master
    float target_vol = master_volume_ * ambient_fade_target_;
